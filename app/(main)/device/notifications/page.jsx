...
const Dashboard = () => {
    ...
    const { latestLog, isConnected, lastConnectionUpdate, sendMessageAndWaitForCondition } = useContext(WebsocketContext);
    ...
    return (
        <div className="grid fadeIn">
            ...
            <Dialog
                ...
                draggable={true}
                resizable={false}
                footer={() => <Button className={'w-full'} label="Close" onClick={() => setViewDialogVisible(false)} />}
            >
            ...
            <div className="col-12">
                <div className="card mb-0">
                    <DataView
                        ...
                        paginatorLeft={() => {
                            return <Button icon="pi pi-refresh" text loading={loading} disabled={!isConnected || data?.status !== 'CONNECTED'} onClick={reloadData} />;
                        }}
                        loading={loading}
                        paginator={true}
                        value={logs}
                        listTemplate={(items, options) => {
                            ...
                            let list = items
                                .filter((product) => product?.PRIORITY && product?.MESSAGE)
                                .map((product, index) => {
                                    return (
                                        <div key={index} className={'col-12'}>
                                            <CustomMessage
                                                __REALTIME_TIMESTAMP={product?.__REALTIME_TIMESTAMP}
