...
const Dashboard = () => {
    ...
    return (<div className="grid">
            ...
            <div className="card mb-0">
                <div className="flex justify-content-between mb-3">
                    <div>
                        ...
                        <div
                            className={'text-900 font-medium text-xl ' + (isConnected ? infoData?.connected ? infoData?.networkLatencyMS : 'text-red-600 animate-pulse' : 'text-red-600 animate-pulse')}>{isConnected ? infoData?.connected ? infoData?.networkLatencyMS || 'Unknown' : 'Disconnected' : 'Disconnected'}</div>
                    </div>
                    <div className="flex align-items-center justify-content-center bg-blue-100 border-round"
                         style={{ width: '2.5rem', height: '2.5rem' }}>
            ...
            <div className="card mb-0">
                <div className="flex justify-content-between mb-3">
                    <div>
                        ...
                        <div
                            className={'text-900 font-medium text-xl ' + (isConnected ? infoData?.connected ? infoData?.info?.totalMessages : 'text-red-600 animate-pulse' : 'text-red-600 animate-pulse')}>{isConnected ? infoData?.connected ? infoData?.info?.totalMessages?.toLocaleString() ?? 'Unknown' : 'Disconnected' : 'Disconnected'}</div>
                    </div>
                    <div className="flex align-items-center justify-content-center bg-blue-100 border-round"
                         style={{ width: '2.5rem', height: '2.5rem' }}>
        ...
        <div className="col-12">
            <div className="card mb-0">
                <h5>CPU Usage</h5>
                <ProgressBar value={isConnected && infoData?.connected
                    ? (infoData.info.processCpuLoadPercentage ?? 0).toFixed(2)
                    : 0}></ProgressBar>
                <h5>Memory Usage</h5>
                <ProgressBar
                    value={isConnected && infoData?.connected
                        ? ((infoData.info.usedMemoryMB / infoData.info.maxMemoryMB) * 100).toFixed(2)
                        : 0}></ProgressBar>
                <h5>Total Threads</h5>
                <ProgressBar displayValueTemplate={(value) => {
                    return (<React.Fragment>
                        {value}
                    </React.Fragment>);
                }} value={infoData?.info?.totalThreads || 0}></ProgressBar>
                <h5>Available Processors</h5>
                <ProgressBar displayValueTemplate={(value) => {
                    return (<React.Fragment>
                        {value}
                    </React.Fragment>);
                }} value={infoData?.info?.availableProcessors || 0}></ProgressBar>
            </div>
