...
const Dashboard = () => {
        ...
        const intervalId = setInterval(() => {
            if (isConnected) {
                sendMessageAndWaitForCondition({ type: 'DEVICES-DATA' }, (m) => m.type === 'DEVICES-DATA')
                    .then((message) => {
                        setXtableStatus(message?.message?.xtablesConnectedStatus || 'UNKNOWN');
                        setLogs((a) => {
                                ...
                                <div className="card mb-0">
                                    <div className="flex justify-content-between mb-3">
                                        <div>
                                            ...
                                            <div className={'text-900 font-medium text-xl text-green-600'}>{isConnected ? data?.server ?? 'Unknown' : 'Unknown'}</div>
                                        </div>
                                ...
                                <div className="card mb-0">
                                    <div className="flex justify-content-between mb-3">
                                        <div>
                                            ...
                                            <div className={'text-900 font-medium text-xl text-green-600'}>{isConnected ? data?.hostname ?? 'Unknown' : 'Unknown'}</div>
                                        </div>
                                        <div className="flex align-items-center justify-content-center bg-blue-100 border-round" style={{ width: '2.5rem', height: '2.5rem' }}>
                                ...
                                <div className="card mb-0">
                                    <div className="flex justify-content-between mb-3">
                                        <div>
                                            ...
                                            <div className={'text-900 font-medium text-xl text-green-600'}>{isConnected ? data?.address ?? 'Unknown' : 'Unknown'}</div>
                                        </div>
                                        <div className="flex align-items-center justify-content-center bg-blue-100 border-round" style={{ width: '2.5rem', height: '2.5rem' }}>
                                ...
                                <div className="card mb-0">
                                    <div className="flex justify-content-between mb-3">
                                        <div>
                                            ...
                                            <div className={'text-900 font-medium text-xl ' + (data?.status === 'CONNECTED' ? 'text-green-600' : data?.status === 'CONNECTING' ? 'text-yellow-500 animate-pulse-fast' : 'text-red-600 animate-pulse')}>
                                                {isConnected ? data?.status ?? 'Unknown' : 'Unknown'}
                                            </div>
                                        </div>
                                        <div className="flex align-items-center justify-content-center bg-blue-100 border-round" style={{ width: '2.5rem', height: '2.5rem' }}>
                                            <i className="pi pi-chevron-circle-up text-blue-500 text-xl" />
    ...
    return (
        <div className="grid fadeIn">
            <Toast ref={toast} />
            <Dialog maximizable={false} closable={false} maximized={false} header="Machine Auto Discovery" visible={dialogVisible} onHide={handleStop} footer={<Button className={'w-full'} label="Stop" onClick={handleStop} />}>
            ...
            <Dialog
                ...
                onHide={() => setAddDeviceDialogVisible(false)}
            ...
            >
                ...
                <InputText placeholder={'Machine Hostname'} className={'w-full'} value={addDeviceHostnameInput} onChange={(e) => setAddDeviceHostnameInput(e.target.value)} />

                <Divider align="center">
                    <Badge value="Address"></Badge>
                </Divider>

                <InputText placeholder={'Machine Address'} className={'w-full'} value={addDeviceAddressInput} onChange={(e) => setAddDeviceAddressInput(e.target.value)} />
                <Divider align="center">
                    <Badge value="Username"></Badge>
                </Divider>

                <InputText placeholder={'Machine Username'} className={'w-full'} value={addDeviceUsernameInput} onChange={(e) => setAddDeviceUsernameInput(e.target.value)} />
                <Divider align="center">
                    <Badge value="Password"></Badge>
                </Divider>

                <InputText placeholder={'Machine Password'} className={'w-full'} value={addDevicePasswordInput} onChange={(e) => setAddDevicePasswordInput(e.target.value)} />
            </Dialog>
                ...
                <div className="card mb-0">
                    <div className="flex justify-content-between mb-3">
                        <div>
                            ...
                            <div className="text-900 font-medium text-xl">{isConnected ? xtableStatus : 'Disconnected'}</div>
                        </div>
                        <div className="flex align-items-center justify-content-center bg-blue-100 border-round" style={{ width: '2.5rem', height: '2.5rem' }}>
                ...
                <div className="card">
                    <DataTable
                        ...
                        paginatorLeft={() => {
                            return (
                                <>
                                    <Button icon="pi pi-refresh" text loading={loading} disabled={!isConnected} onClick={startScanning} />
                                    <Button
                                        icon="pi pi-plus"
                                        text
                                        loading={loading}
                    ...
                    >
                        ...
                        <Column
                            ...
                            body={(data) => (
                                <>
                                    <Button
                                        loading={loadingStates[data?.server]}
                ...
                <div className="card">
                    <div className="overflow-y-auto" style={{ maxHeight: '40rem' }}>
                        {isConnected && (logs ?? []).length > 0
                            ? logs.map((log, index) => <LogComponent key={index} log={log}></LogComponent>)
