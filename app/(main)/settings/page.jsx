...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        if (JSON.stringify(copyData) !== JSON.stringify(copyNewData)) setSaveChangesDialogVisible(true);
        else setSaveChangesDialogVisible(false);
    }, [newData, data]);
    ...
    return (
        <div className="grid fadeIn">
            <Dialog
                ...
                footer={() => (
                    <div>
                        <Button loading={loading} disabled={!isConnected} label="Save" icon="pi pi-check" onClick={set} />
                        <Button
                            loading={loading}
                ...
                visible={saveChangesDialogVisible}
                onHide={() => {
                ...
                <div className="card mb-0">
                    <div className="flex justify-content-between mb-3">
                        <div>
                            ...
                            <div className="text-900 font-medium text-xl">{isConnected ? data?.LAST_UPDATED ? <TimeAgo date={data?.LAST_UPDATED} /> : 'Unknown' : 'Disconnected'}</div>
                        </div>
                        <div className="flex align-items-center justify-content-center bg-blue-100 border-round" style={{ width: '2.5rem', height: '2.5rem' }}>
                ...
                <div className="card mb-0">
                    <div className="flex justify-content-between mb-3">
                        <div>
                            ...
                            <div className="text-900 font-medium text-xl">{isConnected ? data?.LAST_RELOADED ? <TimeAgo date={data?.LAST_RELOADED} /> : 'Unknown' : 'Disconnected'}</div>
                        </div>
                        <div className="flex align-items-center justify-content-center bg-blue-100 border-round" style={{ width: '2.5rem', height: '2.5rem' }}>
                ...
                <div className="card">
                    <div className={'grid mt-1'}>
                        <div className={'col-12 lg:col-6'}>
                            <Button disabled={!isConnected} loading={loading} onClick={save} outlined={true} label={'Save'} severity={'warning'} className={'w-full'} />
                        </div>
                        <div className={'col-12 lg:col-6'}>
                            <Button disabled={!isConnected} loading={loading} onClick={reload} label={'Reload'} severity={'info'} className={'w-full'} />
                        </div>
                        <div className={'col-12'}>
                            <Button outlined={true} disabled={!isConnected} loading={loading} onClick={updateFields} label={'Update Fields'} severity={'secondary'} className={'w-full'} />
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <TabView scrollable={true}>
                        <TabPanel disabled={!isConnected || loading} className={'w-full'} header="RIO" leftIcon="pi pi-microchip mr-2">
                            ...
                            <InputText
                                ...
                                value={newData?.ROBORIO_HOSTNAME}
                                onChange={(e) =>
                                    setNewData((prev) => ({
                                        ...prev,
                            ...
                            <InputText
                                ...
                                value={newData?.ROBORIO_USERNAME}
                                onChange={(e) =>
                                    setNewData((prev) => ({
                                        ...prev,
                        ...
                        <TabPanel disabled={!isConnected || loading} className={'w-full'} header="Machines" leftIcon="pi pi-users mr-2">
                            ...
                            <InputText
                                className={'w-full'}
                                placeholder={'Username'}
                                value={newData?.SERVERS_USERNAME}
                                onChange={(e) =>
                                    setNewData((prev) => ({
                                        ...prev,
                            ...
                            <InputText
                                className={'w-full'}
                                placeholder={'Password'}
                                value={newData?.SERVERS_PASSWORD}
                                onChange={(e) =>
                                    setNewData((prev) => ({
                                        ...prev,
                            ...
                            <InputText
                                invalid={!isValidPath(newData?.SYNC_DIRECTORY)}
                                className={'w-full'}
                                placeholder={'Host Directory'}
                                value={newData?.SYNC_DIRECTORY}
                                onChange={(e) =>
                                    setNewData((prev) => ({
                                        ...prev,
                            ...
                            <InputText
                                invalid={!isValidPath(newData?.SYNC_TARGET_DIRECTORY)}
                                className={'w-full'}
                                placeholder={'Target Directory'}
                                value={newData?.SYNC_TARGET_DIRECTORY}
                                onChange={(e) =>
                                    setNewData((prev) => ({
                                        ...prev,
                            ...
                            <div className="p-fluid">
                                <Chips
                                    ...
                                    value={newData?.PHOTONVISION_COPROCESSOR_HOSTNAMES}
                                    onChange={(e) =>
                                        setNewData((prev) => {
                        ...
                        <TabPanel disabled={!isConnected || loading} className={'w-full'} header="Docker" leftIcon="pi pi-image mr-2">
                            ...
                            <InputText
                                invalid={!isValidPath(newData?.PROJECT_DIRECTORY)}
                                className={'w-full'}
                                placeholder={'Directory'}
                                value={newData?.PROJECT_DIRECTORY}
                                onChange={(e) =>
                                    setNewData((prev) => ({
                                        ...prev,
                            ...
                            <InputText
                                invalid={!isValidPath(newData?.DOCKER_IMAGES_DIRECTORY)}
                                className={'w-full'}
                                placeholder={'This is where the images are saved.'}
                                value={newData?.DOCKER_IMAGES_DIRECTORY}
                                onChange={(e) =>
                                    setNewData((prev) => ({
                                        ...prev,
                            ...
                            <InputText
                                invalid={!isValidFile(newData?.DOCKER_COMPOSE_FILE_DIRECTORY, 'yml')}
                                className={'w-full'}
                                placeholder={'File Directory'}
                                value={newData?.DOCKER_COMPOSE_FILE_DIRECTORY}
                                onChange={(e) =>
                                    setNewData((prev) => ({
                                        ...prev,
                        ...
                        <TabPanel disabled={!isConnected || loading} className={'w-full'} header="XTABLES" leftIcon="pi pi-table mr-2"></TabPanel>

                        <TabPanel disabled={!isConnected || loading} className={'w-full'} header="Mapping" leftIcon="pi pi-map mr-2"></TabPanel>
                        <TabPanel disabled={!isConnected || loading} className={'w-full'} header="Preferences" leftIcon="pi pi-user mr-2"></TabPanel>
...
function getWifiSVG(strength) {
    if (strength >= 66)
        return (
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                <path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 18h.01m-2.838-2.828a4 4 0 0 1 5.656 0m-8.485-2.829a8 8 0 0 1 11.314 0" />
            </svg>
        );
    ...
    if (strength >= 33)
        return (
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                <path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 18h.01m-2.838-2.828a4 4 0 0 1 5.656 0" />
            </svg>
        );
    return (
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
            <path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 18h.01" />
        </svg>
    );
}
