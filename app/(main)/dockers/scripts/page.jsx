            ...
            .then((msg) => {
                ...
                if (msg?.message?.success === false) {
                    ...
                    countdown(5, (s) => {
                        ...
                        sendMessageAndWaitForConditionWithManage('DOCKER-IMPORT-ALT-BASE', {
    ...
    return (
        <div className="grid fadeIn">
            ...
            <Dialog draggable={false}
                ...
                <TerminalDisplay messages={finalResponseData ?? []} placeholder={'Waiting for messages'}
                                 loadingDots={true} />
            ...
            <Dialog closable={false} draggable={false}
                ...
                <Button loading={loading}
                        onClick={() => setImportBaseImageDialogVisible(false)}
                        label={'Okay'}
                        className={'w-full'}></Button>
            )}>
                <TerminalDisplay messages={finalResponseData?.MESSAGES ?? []} placeholder={'Waiting for messages'}
                                 loadingDots={true} />
                <ProgressBar color={finalResponseData?.SUCCESS === false ? "RED" : finalResponseData?.SUCCESS === true?  "GREEN" : "YELLOW"} displayValueTemplate={ (value) => {
                    return (
                      <>
                          {finalResponseData?.SUCCESS === false ? "ERROR" : (value + "%")}
                      </>
                    );
                }} value={finalResponseData?.SUCCESS === false ? 100: finalResponseData?.PERCENTAGE} className={"w-full mt-3"}/>
                ...
                <div
                    ...
                    <Button onClick={() => {
                        importDockerImage();
                            ...
                            disabled={!isConnected} label={'Execute'}
