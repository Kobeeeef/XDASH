    ...
    return (
        <div className="grid fadeIn">
            ...
            <div className={'col-12'}>
                <div className="card">
                    <DataTable size={'normal'} rowsPerPageOptions={[5, 10, 25, 50]} removableSort value={isConnected ? data : []} emptyMessage={Loader({ message: isConnected ? 'Searching for services' : 'Connecting to backend' })} rows={5} paginator>
                        <Column frozen={true} field="serviceName" filter header="Name" style={{ width: '16%' }} />
                        <Column
                            field="address"
                            body={(d) => (
                                <Button tooltip={'Open in new tab'} tooltipOptions={{ showDelay: 100, position: 'top', mouseTrack: true }} label={d?.address} link onClick={() => window.open('http://' + d?.address + ':' + d?.port, '_blank')} />
                            )}
                            header="Address"
                            style={{ width: '16%' }}
                        />
