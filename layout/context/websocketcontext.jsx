...
export const WebSocketProvider = ({ children, url }) => {
    ...
    const sendMessageAndWaitForConditionWithManage = useCallback(
        (id, message, conditionFunc, timeout = 1500) => {
            if (!isConnected) {
    ...
    const sendMessageAndWaitForCondition = useCallback(
        (message, conditionFunc, timeout = 1500) => {
            if (!isConnected) {
                return Promise.reject(new Error('WebSocket is not connected'));
            }
